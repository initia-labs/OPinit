...
export class L2Monitor extends Monitor {
    ...
    await this.db.transaction(async (transactionalEntityManager: EntityManager) => {
      ...
      for (const evt of events) {
        ...
        const l2Denom = Buffer.from(data['l2_token'])
          .toString()
          .replace('native_', '');
        const coin = await transactionalEntityManager.getRepository(CoinEntity).findOne({
          where: { l2Denom }
        });
  ...
  public async handleBlock(): Promise<void> {
    await this.db.transaction(async (transactionalEntityManager: EntityManager) => {
      ...
      const lastOutput = await transactionalEntityManager.getRepository(OutputEntity).find({
      ...
      const txEntities = await transactionalEntityManager.getRepository(TxEntity).find({
      ...
      await transactionalEntityManager.getRepository(OutputEntity).save(outputEntity);
