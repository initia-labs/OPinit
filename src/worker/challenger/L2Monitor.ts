...
export class L2Monitor extends Monitor {
  ...
  public async run(): Promise<void> {
    try {
      await this.db.transaction(async (transactionalEntityManager: EntityManager) => {
        const lastOutput = await this.getLastOutputFromDB(transactionalEntityManager);
      ...
      for (const evt of events) {
        ...
        switch (attrMap['type_tag']) {
          case '0x1::op_bridge::TokenBridgeInitiatedEvent': {
            ...
            await transactionalEntityManager.getRepository(WithdrawalTxEntity).save(tx);
            break;
          }
          case '0x1::op_bridge::TokenBridgeFinalizedEvent': {
            ...
            await transactionalEntityManager.getRepository(DepositTxEntity).update(
              { sequence: depositTx.sequence, coinType: depositTx.coinType },
  ...
  async getDepositTx(
    ...
    return await transactionalEntityManager.getRepository(DepositTxEntity).findOne({
  ...
  public async handleBlock(): Promise<void> {
    await this.db.transaction(async (transactionalEntityManager: EntityManager) => {
      ...
      const txEntities = await transactionalEntityManager.getRepository(WithdrawalTxEntity).find({
      ...
      for (const entity of txEntities) {
        ...
        await transactionalEntityManager.getRepository(WithdrawalTxEntity).save(entity);
      }
      ...
      await transactionalEntityManager.getRepository(ChallengerOutputEntity).save(outputEntity);
