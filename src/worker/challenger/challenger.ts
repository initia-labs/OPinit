...
import { Wallet, MnemonicKey, BCS } from '@initia/initia.js';
...
export class Challenger {
  ...
  async getSyncedHeight(monitorName: string, syncedHeightProperty: string) {
    ...
    if (!state) {
      await this.db
        ...
        .save({ name: monitorName, height: 0 });
    }
  ...
  private async monitorTransactions(
    ...
    for (;;) {
      ...
      await this.db
        .getRepository(StateEntity)
        .update({ name: monitorName }, { height: this[syncedHeightProperty] });
  ...
  public async monitorL1Deposit() {
    await this.monitorTransactions(
      ...
      (txs: L1TokenBridgeInitiatedEvent[], height) =>
        txs.map((tx) => {
  ...
  public async monitorL2Withdrawal() {
    await this.monitorTransactions(
      ...
      (txs: L2TokenBridgeInitiatedEvent[], height) =>
        txs.map((tx) => {
  ...
  async makeL2OutputRoot(
    outputIndex: number,
    lastL2BlockHeight: number,
    storage: WithdrawalStorage
  ): Promise<string> {
    const res = await axios.get(
      `${config.L2_LCD_URI}/cosmos/base/tendermint/v1beta1/blocks/${lastL2BlockHeight}`
    );
    ...
    const challengerOutputRoot = createOutputRoot(
      version,
      stateRoot,
      storageRoot,
      lastBlockHash
  ...
  public async getDepositTxEvents(
    height: number
  ): Promise<L1TokenBridgeInitiatedEvent[] | null> {
    const res = await axios.get(
      `${config.L1_LCD_URI}/cosmos/tx/v1beta1/txs?events=tx.height=${height}`
    );
      ...
      logs.events.forEach((event) => {
        ...
        for (const attr of event.attributes) {
          if (attr.key === 'type_tag' && attr.value === evtName) {
            const dataAttr = event.attributes.find(
              (attr) => attr.key === 'data'
            );
            if (!dataAttr) {
              return null;
            }
            const events: L1TokenBridgeInitiatedEvent = JSON.parse(
              dataAttr.value
            );
  ...
  public async getWithdrawalTxEvents(
    height: number
  ): Promise<L2TokenBridgeInitiatedEvent[] | null> {
    const res = await axios.get(
      `${config.L2_LCD_URI}/cosmos/tx/v1beta1/txs?events=tx.height=${height}`
    );
      ...
      logs.events.forEach((event) => {
        ...
        for (const attr of event.attributes) {
          if (attr.key === 'type_tag' && attr.value === evtName) {
            ...
            const events: L2TokenBridgeInitiatedEvent = JSON.parse(
              dataAttr.value
            );
