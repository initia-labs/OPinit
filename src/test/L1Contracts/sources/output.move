...
module addr::op_output {
    ...
    struct ConfigStore<phantom L2ID> has copy, key {
    ...
    const EL2_BLOCK_NUM_MISMATCH: u64 = 7;

    // hash bytes vector length should be 32
    const EINVALID_HASH_LENGTH: u64 = 8;

    // out of output index 
    const EOUT_OF_OUTPUT_INDEX: u64 = 9;
    ...
    public entry fun initialize<L2ID>(
        ...
        assert!(!exists<ConfigStore<L2ID>>(account_addr), ECONFIG_STORE_ALREADY_EXISTS);
        assert!(!exists<OutputStore<L2ID>>(account_addr), EOUTPUT_STORE_ALREADY_EXISTS);
        assert!(l2_address<L2ID>() == account_addr, EL2_ADDRESS_MISMATCH);
    ...
    public entry fun propose_l2_output<L2ID>(
        ...
        let l2_addr = l2_address<L2ID>();
        let config_store = borrow_global<ConfigStore<L2ID>>(l2_addr);
        assert!(signer::address_of(account) == config_store.proposer, EPROPOSER_ADDRESS_MISMATCH);
        ...
        let next_block_num = if (next_output_index == 0) {
            ...
            assert!(table::prepare<u64, OutputProposal<L2ID>>(&mut iter), EUNKNOWN);
        ...
        assert!(l2_block_number == next_block_num, EL2_BLOCK_NUM_MISMATCH);
        assert!(vector::length(&output_root) == 32, EINVALID_HASH_LENGTH);
    ...
    public entry fun delete_l2_output<L2ID>(
        ...
        let account_addr = signer::address_of(account);
        let config_store = borrow_global<ConfigStore<L2ID>>(account_addr);
        assert!(account_addr == config_store.proposer, EPROPOSER_ADDRESS_MISMATCH);

        let output_store = borrow_global_mut<OutputStore<L2ID>>(account_addr);
        let next_output_index = table::length(&output_store.outputs);
        assert!(output_index < next_output_index, EOUT_OF_OUTPUT_INDEX);
