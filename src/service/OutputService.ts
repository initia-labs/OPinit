...
export async function getOutput(outputIndex: number): Promise<GetOutputResponse> {
  const [db] = getDB();
...
export async function getAllOutputs(param:GetAllOutputsParam): Promise<GetAllOutputsResponse> {
  const [db] = getDB();
  const queryRunner = db.createQueryRunner('slave');
