...
export class RPCSocket {
  ...
  public connect(): void {
    this.disconnect()
    this.ws = new Websocket(this.wsUrl.replace('http', 'ws') + '/websocket')
...
async function getRequest(rpc: string, path: string, params?: Record<string, string>): Promise<any> {
...
export async function getBlockBulk(start: string, end: string): Promise<BlockBulk | null>{
    const blockBulksResult: BlockBulk = await getRequest(config.L2_RPC_URI, `/block_bulk`, { start, end })
 ...
 */
export async function lookupInvalidBlock(rpcUrl: string): Promise<InvalidBlock | null>{
    const invalidBlockResult: InvalidBlock = await getRequest(rpcUrl, `/invalid_block`)
