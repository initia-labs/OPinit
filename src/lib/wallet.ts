...
import { Key, Wallet, Msg, TxInfo, MnemonicKey, LCDClient } from '@initia/minitia.js'
...
export function initWallet(
  ...
  switch (type) {
    case WalletType.Challenger:
      wallets[type] = new TxWallet(lcd, new MnemonicKey({ mnemonic: config.CHALLENGER_MNEMONIC }));
      break;
    case WalletType.Executor:
      wallets[type] = new TxWallet(lcd, new MnemonicKey({ mnemonic: config.EXECUTOR_MNEMONIC }));
      break;
    case WalletType.BatchSubmitter:
      wallets[type] = new TxWallet(lcd, new MnemonicKey({ mnemonic: config.BATCH_SUBMITTER_MNEMONIC }));
      break;
    case WalletType.OutputSubmitter:
      wallets[type] = new TxWallet(lcd, new MnemonicKey({ mnemonic: config.OUTPUT_SUBMITTER_MNEMONIC }));
      break;
  }
