...
func Test_TrackWithdrawalCommitments(t *testing.T) {
	...
	err = input.OPChildKeeper.SetWithdrawalCommitment(ctx, 2, types.WithdrawalCommitment{
		Commitment: types.CommitWithdrawal(1, "recipient", sdk.NewInt64Coin("uinit", 100)),
		SubmitTime: now,
	})
	require.NoError(t, err)
	...
	err = input.OPChildKeeper.SetWithdrawalCommitment(ctx, 3, types.WithdrawalCommitment{
		Commitment: types.CommitWithdrawal(1, "recipient", sdk.NewInt64Coin("uinit", 100)),
	...
	require.NoError(t, err)
	_, err = input.OPChildKeeper.GetWithdrawalCommitment(ctx, 2)
	require.NoError(t, err)
	_, err = input.OPChildKeeper.GetWithdrawalCommitment(ctx, 3)
	require.NoError(t, err)
